...
const { rank, title, author, date } = item;
</script>
...
<template>
  <div class="trending-card" :class="{ 'active-trending-card': active }">
    ...
    <p class="trending-date">{{ date }}</p>
  </div>
</template>

<style scoped>
.trending-card {
  @apply tw-relative tw-min-w-[16rem] tw-h-[16rem] tw-flex tw-flex-col tw-gap-7 tw-justify-center tw-text-center tw-rounded-lg tw-border-2 dark:tw-border-slate-600 tw-transition-all tw-duration-200 tw-p-2;
}
...
.trending-date {
  @apply tw-font-thin tw-text-sm dark:tw-text-slate-400;
}

.active-trending-card {
  @apply tw-min-w-[19rem] tw-h-[21rem] dark:tw-border-primary tw-gap-11 tw-p-3;
}
...
.active-trending-card .trending-date {
  @apply tw-text-base;
}

/* [data-active='true'] {
  @apply tw-border-2 dark:dark:tw-border-primary;
} */
</style>
